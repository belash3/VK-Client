...
//  VK Client
//
//  Created by Сергей Беляков on 18.07.2021.
//
...
class NewsResponse: Codable {
    ...
    init(items: [NewsItem]?, profiles: [Profile]?, groups: [NewsGroup]?, nextFrom: String?) {
        self.items = items
...
class Attachment: Codable {
    ...
    init(type: String?, photo: NewsPhoto?, link: Link?) {
        self.type = type
        self.photo = photo
        self.link = link
    }
...
class Link: Codable {
    ...
    init(url: String?, title: String?, caption: String?, linkDescription: String?, photo: NewsPhoto?, amp: Amp?, target: String?, isFavorite: Bool?) {
        self.url = url
        self.title = title
